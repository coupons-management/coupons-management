...
public class CouponServiceImpl implements CouponService {
     ...
     * 优惠券手动排序
     */
    public void updateSort(MannulSortRequest mannulSortRequest) {
        if (null == mannulSortRequest) {
            throw new BusinessRuntimeException("参数不能为空");
        }
        final int storeId = mannulSortRequest.getStoreId();
        if(storeId <= 0 ){
            throw new BusinessRuntimeException("商家ID不能为空");
        }
        cpCouponDAO.cleanManualSort(storeId);
        List<CouponSortReq> sortList = mannulSortRequest.getSortList();
        sortList.forEach(e -> {
